...
const API_KEY = "77632efe5996475b8ba53c95199da364";
function speak(joke){
    VoiceRSS.speech({
        key: API_KEY,
        src: joke,
...
// Using Joke API
const jokeAPI = "https://v2.jokeapi.dev/joke/Dark";
async function getJoke() {
    let joke = '';
    try{
        const response = await fetch(jokeAPI);
        const data = await response.json();
        if(data.type === 'twopart') joke = `${data.setup} ... ${data.delivery}`;
        else joke = data.joke;
        // Text_to_Speech
        speak(joke);
        // Disble Button
        ...
        alert("Something is Wrong With API request");
    }
}
